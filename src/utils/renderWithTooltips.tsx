...
const JARGON: Record<string, string> = {
  ...
  'Test-Time Compute':
    '推理期算力扩展：给予模型更多时间在内部进行"反思"，大幅提升其在复杂逻辑任务上的上限。',
...
type TooltipPart = { type: 'tooltip'; term: string; desc: string };
type Part = TextPart | TooltipPart;

export function renderWithTooltips(text: string | undefined | null): React.ReactNode {
  if (!text) return null;
