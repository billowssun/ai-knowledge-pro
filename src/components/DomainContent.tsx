...
const DomainContent = memo(function DomainContent({ activeDomain }: DomainContentProps) {
  ...
  return (
    <section className="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-[2.5rem] p-6 sm:p-10 border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden">
      <div className={`absolute top-0 right-0 w-80 h-80 ${styles.bgBlur} blur-[100px] -mr-32 -mt-32`}></div>
      <div className="relative z-10">
        <div className="flex flex-col gap-4 mb-6 sm:mb-8">
          <div className="flex items-start gap-4">
            <div className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl ${styles.bgLight} ${styles.textMain} shadow-sm shrink-0 dark:bg-indigo-500/10 dark:text-indigo-400`}>
              <IconComponent className="w-5 h-5" aria-hidden="true" />
            </div>
            <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-slate-900 dark:text-white tracking-tight leading-tight pt-1">{activeDomain?.principles?.title}</h2>
          </div>
        ...
        {hasSimple && (
          <div className={`relative mb-4 p-4 sm:p-5 rounded-xl sm:rounded-2xl ${styles.bgLight} dark:bg-indigo-500/5 border border-indigo-100 dark:border-indigo-500/20`}>
        ...
        {hasSimple ? (
          ...
          >
            <summary className="flex items-center gap-2 text-xs sm:text-sm font-black text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer list-none uppercase tracking-widest py-2">
              <ChevronDown
          ...
          {activeDomain?.stats?.map((stat) => {
            ...
            <div key={stat.id} className="space-y-2 sm:space-y-3">
              <div className="flex justify-between items-center text-[10px] sm:text-xs font-black text-slate-500 uppercase tracking-wider sm:tracking-widest">
                ...
                <span className="text-slate-900 dark:text-slate-200 shrink-0">{displayValue}</span>
              </div>
