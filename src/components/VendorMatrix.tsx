      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5 sm:gap-3" role="tablist">
        {vendors.map((v, idx: number) => {
          ...
          >
            ...
            {activeVendor?.id === v.id && <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2"><Star className="w-3 h-3 text-indigo-500 fill-indigo-500" aria-hidden="true" /></div>}
          </button>
        )})}
        <div className="flex flex-col items-center justify-center p-3 sm:p-5 rounded-xl sm:rounded-[1.5rem] border border-dashed border-slate-200 opacity-40" aria-hidden="true">
  ...
  const content = useMemo(() => {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 sm:py-20 px-4 bg-white rounded-2xl sm:rounded-[2.5rem] border border-dashed border-slate-200 text-center shadow-sm">
        <Icons.Ghost className="w-10 h-10 sm:w-12 sm:h-12 text-slate-200 mb-4 sm:mb-5 animate-pulse" aria-hidden="true" />
  ...
  return (
    <section className="space-y-6 sm:space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl shadow-inner" role="group" aria-label="厂商筛选">
          {FILTER_TABS.map((tab) => (
            <button
              key={tab.key}
