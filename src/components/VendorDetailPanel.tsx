...
const VendorDetailPanel = memo(function VendorDetailPanel({ activeVendor, onClose }: VendorDetailPanelProps) {
  ...
  return (
    <div className="bg-slate-50 w-full rounded-2xl sm:rounded-3xl border border-slate-200 shadow-inner relative animate-fade-in mt-6 sm:mt-8" role="tabpanel">
              ...
              <div key={m.name} className="bg-white border-2 border-indigo-50 rounded-2xl sm:rounded-[2rem] p-5 sm:p-8 shadow-sm hover:shadow-xl hover:border-indigo-200 transition-all flex flex-col group relative overflow-hidden">
                 <div className="flex justify-between items-start mb-5 sm:mb-8 relative z-10 gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] sm:text-xs font-black text-slate-400 uppercase mt-1 sm:mt-1.5 flex items-center gap-1.5 tracking-widest"><Layers className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" aria-hidden="true"/><span className="truncate">{m.type}</span></div>
                    </div>
                 ...
                 <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-6">
                  {m.status === '开源' ? (
                    <>
                      <span className="text-[9px] sm:text-[10px] font-black bg-emerald-100/50 text-emerald-600 px-2 py-0.5 sm:py-1 rounded-md flex items-center gap-1"><Laptop className="w-3 h-3" aria-hidden="true"/> 可本地部署</span>
                      <span className="text-[9px] sm:text-[10px] font-black bg-cyan-100/50 text-cyan-600 px-2 py-0.5 sm:py-1 rounded-md flex items-center gap-1"><Unlock className="w-3 h-3" aria-hidden="true"/> 权重开放</span>
                    </>
                  ) : (
                    <>
                      <span className="text-[9px] sm:text-[10px] font-black bg-purple-100/50 text-purple-600 px-2 py-0.5 sm:py-1 rounded-md flex items-center gap-1"><Cloud className="w-3 h-3" aria-hidden="true"/> 纯云端 API</span>
                      <span className="text-[9px] sm:text-[10px] font-black bg-amber-100/50 text-amber-600 px-2 py-0.5 sm:py-1 rounded-md flex items-center gap-1"><CreditCard className="w-3 h-3" aria-hidden="true"/> 商业授权</span>
                    </>
                  )}
                 </div>
